...
public:
	...
	vector<Vec3f> getTriangle(int index) {
		assert(index >= 0 && index < num_triangles);
		vector<Vec3f> ans;
		ans.push_back(vertices[triangles[index * 3 + 0]]);
		ans.push_back(vertices[triangles[index * 3 + 1]]);
		ans.push_back(vertices[triangles[index * 3 + 2]]);
		return ans;
	}
