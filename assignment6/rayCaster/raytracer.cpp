	...
	for (int i = 0; i < lightn; i++) {
		...
		//计算与光线之间的第一个与物体的交点存到tmp里
			...
			if(isht)
				...
				RayTree::AddShadowSegment(ptol, 0, FLT_MAX);
		...
		if (!shadows || (shadows && (!isht))) {
			...
			//cout << "no hit"<<tmpcolor << endl;
		}
		else {
			//开启了shadows且有交点(无光线直射)
#if OPTION_TRANSPARENT_SHADOWS
			//处理透明光照
			...
			if(!griduse)
				...
				isht = (g)->intersect(tmpr, tmp, epsilon);
			...
			/*if (tmpcolor != Vec3f(0, 0, 0) && fs) 
				cout << "tmpcolor in loop" << tmpcolor << ' '<<ct<<' '<<tt<<' '<<ft
				<<' '<<tss<<endl;*/
			//cout << "------------\n";
		...
		/*if (tmpcolor != Vec3f(0, 0, 0)&& !flgg)
			cout << "tmpcolor :" << tmpcolor << ' '
			<<  endl;*/
		color += tmpcolor;
	}
	//cout << color << endl;
...
Vec3f RayTracer::traceRay(Ray& ray, float tmin, int bounces, float weight,
	...
	if (reflectiveColor.Length() > 0 &&
		...
		tmpcolor = reflectiveColor * traceRay(refRay, epsilon2, bounces + 1, weight * reflectiveColor.Length()/sqrtf(3.0), indexOfRefraction, ref_hit);
		RayTracingStats::IncrementNumNonShadowRays();
		color += tmpcolor;
		//cout << tmpcolor <<' '<<color<< endl;
		RayTree::AddReflectedSegment(refRay, 0, ref_hit.getT());
	}
	if (transparentColor.Length() > 0 &&
		...
		if (ists)
		{
			...
			color += transparentColor * traceRay(trsRay, epsilon2, bounces + 1, weight * transparentColor.Length()/sqrtf(3.0), index_t, trs_hit);
			RayTracingStats::IncrementNumNonShadowRays();
			RayTree::AddTransmittedSegment(trsRay, 0, trs_hit.getT());
		}
	}
