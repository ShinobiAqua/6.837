...
public:
	...
	void GetInt(int& d0, int& d1, int& d2) const {
		d0 = (int)data[0];
		d1 = (int)data[1];
		d2 = (int)data[2];
	}
	...
	void Clamp(Vec3f low , Vec3f high) {
		if (data[0] < low.x()) data[0] = low.x();  if (data[0] > high.x()) data[0] = high.x();
		if (data[1] < low.y()) data[1] = low.y();  if (data[1] > high.y()) data[1] = high.y();
		if (data[2] < low.z()) data[2] = low.z();  if (data[2] > high.z()) data[2] = high.z();
	}
	...
	friend Vec3f operator/(const Vec3f& v1, const Vec3f& v2) {
		Vec3f v3; Divide(v3, v1, v2); return v3;
	}
	...
	static void Divide(Vec3f& a, const Vec3f& b, const Vec3f& c) {
		a.data[0] = c.data[0] !=0 ? b.data[0] / c.data[0]:FLT_MAX;
		a.data[1] = c.data[1] !=0 ? b.data[1]  / c.data[1]:FLT_MAX;
		a.data[2] = c.data[2] !=0 ? b.data[2] / c.data[2]:FLT_MAX;
	}
	...
	static Vec3f Abs(Vec3f& a) {
		Vec3f tmp = a;
		tmp.Abs();
		return tmp;
	}
	static Vec3f Negate(Vec3f& a) {
		Vec3f tmp = a;
		tmp.Negate();
		return tmp;
	}
