...
public:
	Grid(BoundingBox* bb, int xi, int yi, int zi,bool vs):box(bb),nx(xi)
	,ny(yi),nz(zi),gridvisualize(vs){
		Mt = new Material * [10];
		//cout << "ssb1" << endl;
		for (int i = 0; i < 10; i++) {
		...
		for (int i = 0; i < nx; i++) {
			...
			for (int j = 0; j < ny; j++) {
				obj[i][j] = new  vector<Object3D*>[nz];
			}
		}
		//cout << "ssb4" << endl;
	...
	~Grid() {
		...
		clearTransform();
		//resetPlanes();
		//releaseFlatternGroup();
	}
	/*void releaseFlatternGroup() {
		for (auto tmp : newtransform) {
			if (tmp) delete tmp;
		}
		newtransform.clear();
		if (flatterngroup)
			delete flatterngroup;
	}*/
	...
	void addPlanes(Object3D* obj,Matrix *m) {
		if(m==NULL)
			planes.push_back(obj);
		else {
			Object3D* tmp = new Transform((*m), obj);
			newtransform.push_back(tmp);
			planes.push_back(tmp);
		}
		cout <<"plane size:"<< planes.size() << endl;
	}
	void clearTransform() {
		for (auto tmp : newtransform) {
			delete tmp;
	...
	/*void fit_and_FlatternGroup(Group *grp) {
		releaseFlatternGroup();
		grp->flattern(newtransform,NULL);
		flatterngroup = new Group((int)newtransform.size());
		int ct = 0;
		for (auto tmp : newtransform) {
			flatterngroup->addObject(ct++, tmp);
		}
		cout << newtransform.size() << endl;
		fit_Group(grp);
	}
	void fit_Group(Group* grp) {
		box = grp->getBoundingbox();
		resetLXLYLZ();
	}
	Group* getFlatternGroup() {
		return flatterngroup;
	}*/
	void addTransform(Object3D* obj) {
		newtransform.push_back(obj);
	}
...
private:
	...
	//vector<Object3D*> newtransformplanes; 
	vector<Object3D*> newtransform;
	//Group* flatterngroup;
};
#endif
